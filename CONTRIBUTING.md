...
When an agent (Grok, Claude, etc.) does the bulk of the implementation work for a set of changes, include a `Co-authored-by` trailer in the commit message. This ensures the agent's contribution is properly credited on GitHub (the trailer is what GitHub uses to display co-authors, even when the primary `Author` of the commit is the human operator/Codex).
...
- Codex (or operator) performs the actual `git push`.
